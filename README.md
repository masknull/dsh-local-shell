...
- **Attach mode**: if DSH is already listening on 3080 at startup, the app attaches instead of spawning a second one — and never kills an instance it didn't start. When that instance demands the browser token and its printed line is long gone, the shell recovers the token **from the running process's own memory** and navigates with it (accepted only after a real HTTP `303` + `dsh-auth-*` exchange — never on shape alone), so reusing a running DSH no longer ends at the "restart and take over?" prompt; only a failed recovery falls back to that prompt
