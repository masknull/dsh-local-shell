...
- **四遍递进, 锚点优先**:先只认打印行 `token=` 之后的 43 字符(实测同一实例:锚点法候选由数百降到个位数)→ 再扫全内存的极大 base64url 串(先跳过 MEM_IMAGE,映射的 DLL 代码不可能存 JS 字符串,少读约 160MB)→ 全量内存 → 最后去掉尾字符约束兜底(防上游改编码)
- **速度**(实测本机 3080 实例基线:扫描 592.5MB / 656 个候选 / 2.5s):4MB 分块 `ReadProcessMemory` + 多线程 `min(CPU,8)` 从原子队列领区间 + 攒够 16 个候选即 **8 路并行探测**、命中即刻停扫(early stop)。串行探测在数百个候选下会变成主要开销——实测真令牌出现在第 649 次探测上,这个量级必须并行;探测是手写的最小 HTTP/1.1 over TCP,显式带 `Host` 头(缺它必然 401——cookie 名与签名 audience 都由请求 authority 派生),只读响应头
...
- Four passes, most specific first: the 43 characters behind the printed URL's `token=` anchor (measured on a live instance: single-digit candidates instead of hundreds), then every maximal base64url run over private memory (mapped executables cannot hold a JS string, ~160MB less to read), then all memory, then all memory without the tail filter
- Fast (measured baseline on this machine's 3080 instance: 592.5MB scanned, 656 candidates, 2.5s): 4MB `ReadProcessMemory` chunks, `min(CPU,8)` workers pulling ranges off an atomic queue, and a probe round every 16 candidates across 8 lanes so a hit stops the scan immediately — the real token showed up on the 649th probe, and that volume has to be probed in parallel. Probes are a hand-written minimal HTTP/1.1 exchange with an explicit `Host` header (without it the server always answers 401 — the cookie name and its signed audience both derive from the request authority)
